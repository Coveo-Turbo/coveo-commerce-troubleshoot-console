...
### Sibling-aware product cards

The `Sibling Swatch Demo` product-template preset (`atomic-sibling-swatch-demo`) supports catalogs where each
color remains a distinct product grouped by the OOTB `ec_item_group_id`. It expects a string field named
`style_group_siblings` containing the JSON sibling array generated by the catalog IPE.

The sibling-aware components parse that field client-side and coordinate color selection across the product
image gallery, title/PDP link, price, and size/SKU selector without an additional API request. The original `Color Swatch Demo`
preset remains available for catalogs whose colors and sizes are represented through `product.children`.
